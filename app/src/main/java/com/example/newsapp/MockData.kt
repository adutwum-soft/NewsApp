...
object MockData {
 val topNewsList = listOf<NewsData>(
     ...
     fun Date.getTimeAgo(): String{
         ...
         val currCalender = Calendar.getInstance()

         val currYear = currCalender.get(Calendar.YEAR)
         val currMonth = currCalender.get(Calendar.MONTH)
         val currDay = currCalender.get(Calendar.DAY_OF_MONTH)
         val currHour = currCalender.get(Calendar.HOUR_OF_DAY)
         val currMinute = currCalender.get(Calendar.MINUTE)

         return if (year < currYear){
             val interval = currYear - year
             if (interval == 1) "$interval year ago" else "$interval years ago"
         }else if (month < currMonth){
             val interval = currMonth - minute
             if (interval == 1) "$interval month ago" else "$interval months ago"
         }else if (day < currDay){
             val interval = currDay - day
             if (interval == 1) "$interval day ago" else "$interval days ago"
         }else if (hour < currHour){
             val interval = currHour - hour
             if (interval == 1) "$interval hour ago" else "$interval hours ago"
         }else if (minute < currMinute){
             val interval = currMinute - minute
